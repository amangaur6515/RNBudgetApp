...
const BudgetListingScreen = () => {
  ...
  const budgetList = useSelector(
    (state) => state.budget.find((m) => m.month === selectedMonth).items
  );

  useEffect(() => {}, [month, budgetList]);
  ...
  return (
    <View>
      ...
      {budgetList.length === 0 ? (
        <View style={styles.emptyBudgetListContainer}>
            ...
            return (
              <View key={item.itemName} style={styles.card}>
                ...
                <View style={styles.amountContainer}>
                  <Text style={[styles.cardText, { color: "teal" }]}>
